...
class DataConfig:
    ...
    num_train_samples: Optional[int] = field(default=None)
    filter_filelist: Optional[str] = field(default=None)
...
class ValidationConfig:
    ...
    pyramid_stage_range: Optional[list[float]] = field(default=None)  # e.g. [0, 0.2, 0.4, 1]
    # ---- Validation mode switches ----
    # Set to False to skip pyramid / hybrid validation entirely.
    # Set to "first_only" to run them only at the first validation step (for reference baseline).
    enable_pyramid_validation: str = field(default="first_only")   # "true" | "false" | "first_only"
    enable_hybrid_validation: str = field(default="false")         # "true" | "false" | "first_only"
    # ---- Multi-chunk generation switches (inside log_validation_edit_adapter) ----
    # Default "true" preserves the original behavior (both run every time).
    # Set to "false" to skip; "first_only" runs them only at the first
    # validation step (handy for a one-time reference and saving time later).
    enable_multichunk_fixed_validation: str = field(default="true")    # "true" | "false" | "first_only"
    enable_multichunk_pyramid_validation: str = field(default="true")  # "true" | "false" | "first_only"
    # ---- Terminal sigma for single-stage DMD ----
    # A 16-step linear schedule ends at σ ≈ 0.19, so the final pred_x0 is taken
    # from a still-noisy input. Two independent knobs move the terminal σ closer
    # to 0; either, both, or neither may be enabled.
    #   final_sigma_floor: replace the schedule's final σ with this smaller
    #     value, keeping the step count. Default None = unchanged.
    #   final_sigma_extra_step: append one extra DMD step at this σ after the
    #     standard schedule. Default None = unchanged.
    final_sigma_floor: Optional[float] = field(default=None)
    final_sigma_extra_step: Optional[float] = field(default=None)
    # ---- Multi-instruction coverage ----
    # When training with multiple data_dirs (edit instructions), validation samples
    # are selected to cover all instructions evenly. When there is only one
    # instruction, falls back to num_validation_videos.
    num_val_samples_per_instruction: int = field(default=2)       # for video generation (expensive)
    num_val_loss_samples_per_instruction: int = field(default=10)  # for val_loss computation (cheap)
...
class TrainingConfig:
    ...
    # ---- Mixture Gaussian sigma sampling (opt-in) ----
    sigma_sampling_strategy: str = field(default="logit_normal")
    mixture_centers: list[float] = field(default_factory=lambda: [0.999, 0.500])
    mixture_stds: list[float] = field(default_factory=lambda: [0.025, 0.025])
    mixture_weights: list[float] = field(default_factory=lambda: [1.0, 1.0])
    raw_final_sigma_floor: Optional[float] = field(default=None)
    mixture_rejection_sampling: bool = field(default=False)
    # ---- Dynamic Shifting ----
    ...
    edit_adapter_dim: int = field(default=1280)
    edit_adapter_num_heads: int = field(default=10)
    edit_adapter_text_dim: int = field(default=4096)
    edit_adapter_eps: float = field(default=1e-6)
    edit_adapter_data_mode: str = field(
        default="splice",
        metadata={"choices": ["splice", "separated"]},
    )
    # ---- Edit Adapter Temporal Self-Attn (opt-in) ----
    # When True, each adapter block applies a temporal-axial self-attn over the
    # current chunk's tokens before the cross-attn.
    edit_adapter_enable_temporal_self_attn: bool = field(default=False)
    # Maximum number of frames the temporal RoPE is precomputed for. Increase
    # if you train on longer chunks (current latent_window covers ~10 frames).
    edit_adapter_temporal_max_len: int = field(default=64)
    # ---- Edit Adapter History Cross-Attn ----
    edit_adapter_history_dim: int = field(default=512)
    edit_adapter_history_num_heads: int = field(default=4)
    edit_adapter_history_num_frames: int = field(default=2)
    # ---- Temporal Loss Reweighting ----
    reweighting_along_time: bool = field(default=False)
    temporal_weight_max_ratio: float = field(default=2.0)
    #
    ...
    stage2_num_inference_steps: int = field(default=8)
